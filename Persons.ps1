...
$MatriculesAExclure = if ($config.matriculesToExclude) {
    $config.matriculesToExclude -split "," | ForEach-Object { $_.Trim() }
} else { @() }

# groupementTerritorialKey : clé ANTIBIA du groupement territorial de rattachement des chefs de CIS SPV
$groupementTerritorialKey = if ($config.groupementTerritorialKey) { $config.groupementTerritorialKey } else { "" }
...
# Filtrage matricules exclus (variabilisé depuis configuration.json → matriculesToExclude)
if ($MatriculesAExclure.Count -gt 0) {
    $persons = $persons | Where-Object { $MatriculesAExclure -notcontains $_.matricule }
    Write-Information "Filtrage matricules exclus : $($MatriculesAExclure -join ', ')"
}
    ...
    foreach ($c in $contractsGrouped["$($p.MATRICULE)"]) {
        ...
        elseif ($managersPoleGrouped["$($c.POLE_CODE)"]) {
            ...
            if ($c.CATEGORIE -like "VOLONTAIRE") {
                ...
                # Chef de centre lui-même → manager = chef du groupement territorial (variabilisé depuis configuration.json → groupementTerritorialKey)
                if ($managerId -eq $p.MATRICULE) {
                    if (-not [string]::IsNullOrWhiteSpace($groupementTerritorialKey)) {
                        $managerId = ($managersGroupementGrouped[$groupementTerritorialKey] | Select-Object -First 1).P_MATR
                    } else {
                        $managerId = $null
                    }
                }
